...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($firstName) || empty($lastName) || empty($email) || empty($password) || empty($address) || empty($userType)) {
        ...
        if ($user->createUser($firstName, $lastName, $email, $password, $address, $userType)) {
            echo "<div class='message-container text-center mt-5 pt-5'><div class='alert alert-success' role='alert'>User has been added successfully.</div></div>";
        } else {
            echo "<div class='message-container text-center mt-5 pt-5'><div class='alert alert-danger' role='alert'>Error adding user.</div></div>";
        }
    }
}
...
<div class="main-container">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        ...
        <button type="submit" class="btn btn-md btn-primary">Add User</button>
    </form>
</div>
